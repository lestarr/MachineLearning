...
#rf<- train(classe~., method = "rf", data = train_clean_nums,  prox=TRUE, trControl = fitControl_parCV2)
load(file = "rf_model.rda")
rf
```

When looking at the model we see that a model with very high accuracy (0.9998) was picked even with the simple 2-fold cross validation
```{r, echo =FALSE, results='hide',message=FALSE, warning=FALSE}
stopCluster(cluster) # close cluster for parallel computing
```

The accuracy of prediction was good, so I decided to leave the model as it is. The confusionMatrix shows that all the classes in the test set were predicted correctly. 
